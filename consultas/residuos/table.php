...
if(!empty($dados_requisicao['search']['value'])) {
    $query_scrap .= " WHERE serial LIKE :serial 
                     ...
                     OR  (SELECT nome FROM usuario WHERE usuario.id = scrap.engenheriro_id ) as engenheiro LIKE :engenheiro
                     OR  (SELECT nome FROM usuario WHERE usuario.id = scrap.coordenador_id ) as coordenador LIKE :coordenador";
}
...
if(!empty($dados_requisicao['search']['value'])) {
    ...
    $result_qnt_scrap->bindParam(':engenheiro', $valor_pesq, PDO::PARAM_STR);
    $result_qnt_scrap->bindParam(':coordenador', $valor_pesq, PDO::PARAM_STR);
}
...
$query_scrap = "SELECT 
    ...
    (SELECT usuario.nome FROM usuario WHERE usuario.id = scrap.coordenador_id LIMIT 1) AS coordenador
FROM scrap;";
...
while ($row_scrap = $result_scrap->fetch(PDO::FETCH_ASSOC)) {
    ...
    $dados[] = [
        ...
        'coordenador' => $row_scrap['coordenador']
    ];
}
