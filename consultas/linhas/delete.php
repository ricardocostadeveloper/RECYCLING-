<?php
    ...
    if (isset($_REQUEST['id']) && is_numeric($_REQUEST['id'])) {
        ...
        $stmt = $conn->prepare("DELETE FROM linha WHERE id = ?");
        $stmt->bind_param("i", $id); // "i" indica que é um inteiro

        if ($stmt->execute()) {
            // Sucesso na exclusão
            echo json_encode(['success' => true, 'message' => 'Linha excluído com sucesso.']);
        } else {
            // Falha na execução da query
            echo json_encode(['success' => false, 'message' => 'Erro ao excluir linha.']);
        }
